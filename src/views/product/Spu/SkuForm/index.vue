...
export default {
  ...
  data() {
    return {
      ...
      // 收集图片的数据字段，因为缺少isDefault字段不能之间赋值给skuInfo.skuImageList
      imageList: [],
    }
  },
  methods: {
    ...
    handleSelectionChange(params) {
      // params是一个数组，收集复选框中选中的数据，但注意缺少isDefault字段
    ...
    async save() {
      ...
      //   //如果有item.attrIdAndValueId这个属性，表示用户进行了选择（复选框那里）
      //   if(item.attrIdAndValueId){
      //     const [attrId,valueId] = item.attrIdAndValueId.split(":");
      //     arr.push({valueId,attrId});
      //   }
      // });
      // // 把整理好的参数赋值给skuInfo.skuAttrValueList
      // skuInfo.skuAttrValueList = arr;

      // 第二种写法:reduce() 方法接收一个函数作为累加器，item就是遍历的元素，[]表示每次的初始值为空数组
